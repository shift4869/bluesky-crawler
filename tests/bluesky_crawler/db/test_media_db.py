...
class TestMediaDB(unittest.TestCase):
    ...
    def get_instance(self) -> MediaDB:
        instance = MediaDB(db_path=":memory:")
        return instance
    ...
    def test_select(self):
        ...
        expect = [Media.create(self.make_params())]
        self.assertEqual(expect, actual)

    def test_upsert(self):
        def get_record(index: int) -> Media:
            return Media.create(self.make_params(index))

        def get_updated_record(index: int) -> Media:
            record = get_record(index)
            args_dict = record.to_dict() | {"registered_at": f"updated_registered_at_{index}"}
            return Media.create(args_dict)

        def strict_check(e_list: list[Media], a_list: list[Media]) -> bool:
            def strict_check_element(e: Media, a: Media) -> bool:
